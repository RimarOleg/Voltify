...
async function loadRecentActions(){
    ...
    try{
        ...
        if(actions.length=== 0){
            container.innerHTML='<p>Дій поки немає</p>';
            return;
        }
        container.innerHTML=actions.map(text=>`<p>${text}</p>`).join('');
    } 
    catch(err){
        console.error("Помилка завантаження дій:", err);
        container.innerHTML='<p>Не вдалося завантажити останні дії</p>';
    }
}
document.addEventListener('DOMContentLoaded', loadRecentActions);
